		...
		mctp_eid_t src, mctp_eid_t dest, void *buf, size_t len)
{
	...
	if(buf != NULL && len >= sizeof(struct mctp_ctrl_msg_hdr))
	{
	    if(((struct mctp_ctrl_msg_hdr*)buf)->msg_type == MCTP_CTRL_HDR_MSG_TYPE)
	    {
       ...
       */
       ...
       && (msg_hdr->command_code < MCTP_CTRL_CMD_FIRST_TRANSPORT))
       {
               if(mctp->control_rx != NULL)
               {
                       ...
                       mctp->control_rx(src, mctp->control_rx_data, buffer, length);
                       return true;
               }
       }
       ...
       && (msg_hdr->command_code <= MCTP_CTRL_CMD_LAST_TRANSPORT))
       {
               ...
               {
                       bus->binding->control_rx(src, bus->binding->control_rx_data, buffer, length);
                       return true;
               }
       }
       else if(msg_hdr->command_code == MCTP_CTRL_CMD_RSVD)
       {
               ...
               */
               if(msg_hdr->request == MCTP_CTRL_HDR_FLAG_REQUEST)
               {
                       struct mctp_ctrl_msg_hdr response_data;
                       memset(&response_data, 0, sizeof(response_data));
                       response_data.command_code = MCTP_CTRL_CMD_RSVD;
                       response_data.completion_code = MCTP_CTRL_CC_ERROR_UNSUPPORTED_CMD;
                       response_data.msg_type = MCTP_CTRL_HDR_MSG_TYPE;
                       response_data.request = 0x0;
...
int mctp_set_transport_rx_ctrl(struct mctp_binding *binding, mctp_rx_fn fn, void *data)
{
