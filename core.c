...
/*
 * @brief Handle control message request.
 * @return true - if request was handled
 * @return false - if the request was not handled (no callback available)
 */
bool mctp_ctrl_handle_msg(struct mctp *mctp, struct mctp_bus *bus,
			  mctp_eid_t src, mctp_eid_t dest, void *buffer,
			  size_t length)
{
	/* TODO: Decide if the custom control callbacks are necessary. */
	/*
