...
static void control_message_callback(mctp_eid_t src, void *data, void *buf,
				     size_t len)
{
...
static int mctp_test_tx(struct mctp_binding *b, struct mctp_pktbuf *pkt)
{
...
static void send_control_message(void)
{
...
static void send_control_message_with_reserved_command_code(void)
{
	struct mctp *endpoint = mctp_init();
	struct mctp_binding binding;
	struct callback_data ctx;
