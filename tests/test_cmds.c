...
static uint8_t callback_results = 0;
static const uint8_t expected_callback_results = 3;
...
void control_message__transport_callback(mctp_eid_t src, void *data, void *buf,
					 size_t len)
{
	struct mctp_ctrl_msg_hdr *msg_hdr = buf;
			...
			struct mctp *test_endpoint)
{
	...
	test_binding->control_rx = control_message__transport_callback;
	test_binding->control_rx_data = NULL;
}
...
int main(int argc, char *argv[])
{
	...
	setup_test_binding(&test_binding, test_endpoint);
