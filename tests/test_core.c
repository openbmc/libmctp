...
static void rx_message(uint8_t eid __unused, bool tag_owner, uint8_t msg_tag, void *data, void *msg __unused,
		       size_t len)
{
...
static void mctp_core_test_rx_with_tag()
{
	...
	flags_seq_tag =
		MCTP_HDR_FLAG_SOM | MCTP_HDR_FLAG_EOM | (1 << MCTP_HDR_TO_SHIFT) | tag;
	receive_one_fragment(binding, test_payload, MCTP_BTU, flags_seq_tag,
			     &pktbuf);
...
static void mctp_core_test_rx_with_tag_multifragment()
{
	...
	flags_seq_tag = MCTP_HDR_FLAG_SOM |
			(get_sequence() << MCTP_HDR_SEQ_SHIFT) | (1 << MCTP_HDR_TO_SHIFT) | tag;
	receive_one_fragment(binding, test_payload, MCTP_BTU, flags_seq_tag,
			     &pktbuf);
