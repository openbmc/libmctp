...
static void astlpc_test_packetised_message_bmc_to_host(void)
{
	...
	/* BMC sends a message */
	rc = mctp_message_tx(ctx.bmc.mctp, 9, 0, false, msg, sizeof(msg));
	assert(rc == 0);
...
static void astlpc_test_simple_message_host_to_bmc(void)
{
	...
	rc = mctp_message_tx(ctx.host.mctp, 8, 0, false, msg, sizeof(msg));
	assert(rc == 0);
...
static void astlpc_test_send_large_packet(void)
{
	...
	rc = mctp_message_tx(host->mctp, 8, 0, false, ctx.msg, 2 * MCTP_BODY_SIZE(8192));
	assert(rc == 0);
	rc = mctp_astlpc_poll(bmc->astlpc);
	assert(rc == 0);
	rc = mctp_astlpc_poll(host->astlpc);
...
static void astlpc_test_tx_before_channel_init(void)
{
	 ...
	 */
	alarm(1);
	mctp_message_tx(bmc->mctp, 9, 0, false, msg, sizeof(msg));
	alarm(0);
