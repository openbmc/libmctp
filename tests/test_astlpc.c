...
static void network_init(struct astlpc_test *ctx)
{
	...
	/* BMC initialisation */
	rc = endpoint_init(&ctx->bmc, MCTP_EID(8), MCTP_BINDING_ASTLPC_MODE_BMC,
	...
	/* Host initialisation */
	rc = endpoint_init(&ctx->host, MCTP_EID(9),
			   MCTP_BINDING_ASTLPC_MODE_HOST, MCTP_BTU, &ctx->kcs,
			   ctx->lpc_mem);
	assert(!rc);
	route = (struct mctp_route){
...
static void astlpc_test_packetised_message_bmc_to_host(void)
{
	...
	/* BMC sends a message */
	rc = mctp_message_tx(ctx.bmc.mctp, MCTP_EID(9), msg, sizeof(msg));
	assert(rc == 0);
...
static void astlpc_test_simple_message_host_to_bmc(void)
{
	...
	rc = mctp_message_tx(ctx.host.mctp, MCTP_EID(8), msg, sizeof(msg));
	assert(rc == 0);
...
static void astlpc_test_host_before_bmc(void)
{
	...
	rc = mctp_register_bus(mctp, &astlpc->binding, MCTP_EID(8));
...
static void astlpc_test_version_host_fails_negotiation(void)
{
	...
	/* BMC initialisation */
	rc = endpoint_init(&bmc, MCTP_EID(8), MCTP_BINDING_ASTLPC_MODE_BMC,
			   MCTP_BTU, &kcs, lpc_mem);
	assert(!rc);
...
static void astlpc_test_simple_indirect_message_bmc_to_host(void)
{
	...
	mctp_register_bus(ctx.bmc.mctp, &ctx.bmc.astlpc->binding, MCTP_EID(8));
	route = (struct mctp_route){
		.range = { .first = 9, .last = 9 },
		.type = MCTP_ROUTE_TYPE_LOCAL,
		.device = { .bus = 0, .address = 0, },
	...
	mctp_register_bus(ctx.host.mctp, &ctx.host.astlpc->binding,
			  MCTP_EID(9));
	route = (struct mctp_route){
		.range = { .first = 8, .last = 8 },
		.type = MCTP_ROUTE_TYPE_LOCAL,
...
static void astlpc_test_buffers_bad_host_init(void)
{
	...
	rc = mctp_register_bus(host.mctp, &host.astlpc->binding, MCTP_EID(8));
	assert(rc < 0);
