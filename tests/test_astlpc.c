...
static void astlpc_test_host_before_bmc(void)
{
	...
	rc = mctp_register_endpoint(mctp, &astlpc->binding, MCTP_EID(8));
...
static void astlpc_test_simple_indirect_message_bmc_to_host(void)
{
	...
	mctp_register_endpoint(ctx.bmc.mctp, &ctx.bmc.astlpc->binding,
			       MCTP_EID(8));
	route = (struct mctp_route){
		.range = { .first = 9, .last = 9 },
		.type = MCTP_ROUTE_TYPE_LOCAL,
	...
	mctp_register_endpoint(ctx.host.mctp, &ctx.host.astlpc->binding,
			       MCTP_EID(9));
	route = (struct mctp_route){
		.range = { .first = 8, .last = 8 },
		.type = MCTP_ROUTE_TYPE_LOCAL,
...
static void astlpc_test_buffers_bad_host_init(void)
{
	...
	rc = mctp_register_endpoint(host.mctp, &host.astlpc->binding,
				    MCTP_EID(8));
	assert(rc < 0);
