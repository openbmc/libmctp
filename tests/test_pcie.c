...
/* please update accordingly to pcie.c source */
...
int main(void)
{
    ...
    pcie = mctp_binding_pcie_init();
    assert(pcie);

    binding = mctp_binding_pcie_core(pcie);
    assert(binding);

    assert(strcmp(pcie->binding.name, "pcie") == 0);
