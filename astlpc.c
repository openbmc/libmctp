...
static int mctp_astlpc_init_fileio_lpc(struct mctp_binding_astlpc *astlpc)
{
	 ...
	 * 🚨🚨🚨
	 *
	 * Decouple ourselves from hiomapd[1] (another user of the FW2AHB) by mapping the FW2AHB
	 * to the reserved memory here as well.
	 *
	 * It's not possible to use the MCTP ASTLPC binding on machines that need the FW2AHB bridge
	 * mapped anywhere except to the reserved memory (e.g. the host SPI NOR).
	 *
	 * [1] https://github.com/openbmc/hiomapd/
	 *
	 * 🚨🚨🚨
	 *
	 * The following calculation must align with what's going on in hiomapd's lpc.c so as not to
	 * disrupt its behaviour:
	 *
	...
	astlpc_prinfo(astlpc, "Configuring FW2AHB to map reserved memory at 0x%08x for 0x%x in the LPC FW cycle address-space",
		      map.addr, map.size);
