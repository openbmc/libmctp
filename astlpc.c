...
static int mctp_astlpc_init_bus_owner(struct mctp_binding_astlpc *astlpc)
{
	...
	hdr = (struct mctp_lpcmap_hdr){
		...
		.rx_offset = htobe32(rx_offset),
		.rx_size = htobe32(rx_size),
		.tx_offset = htobe32(tx_offset),
		.tx_size = htobe32(tx_size),
	};
		...
		struct mctp_pktbuf *pkt)
{
	...
	len_be = htobe32(len);
	mctp_astlpc_lpc_write(astlpc, &len_be, rx_offset, sizeof(len_be));
	mctp_astlpc_lpc_write(astlpc, hdr, rx_offset + 4, len);
...
static void mctp_astlpc_rx_start(struct mctp_binding_astlpc *astlpc)
{
	...
	mctp_astlpc_lpc_read(astlpc, &len, tx_offset, sizeof(len));
	len = be32toh(len);
	...
	mctp_astlpc_lpc_write(astlpc, mctp_pktbuf_hdr(pkt), tx_offset + 4, len);
