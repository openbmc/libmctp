				    ...
				    struct mctp_astlpc_layout *layout)
{
	...
	if (astlpc->mode == MCTP_BINDING_ASTLPC_MODE_BMC) {
		...
		hdr.layout.rx_offset = htobe32(layout->rx.offset);
		hdr.layout.rx_size = htobe32(layout->rx.size);
		hdr.layout.tx_offset = htobe32(layout->tx.offset);
		hdr.layout.tx_size = htobe32(layout->tx.size);
	}
