				   ...
				   struct mctp_astlpc_layout *layout)
{
	...
	if (astlpc->mode == MCTP_BINDING_ASTLPC_MODE_BMC) {
		layout->rx.offset = be32toh(hdr.layout.tx.offset);
		layout->rx.size = be32toh(hdr.layout.tx.size);
		layout->tx.offset = be32toh(hdr.layout.rx.offset);
		layout->tx.size = be32toh(hdr.layout.rx.size);
	} else {
		assert(astlpc->mode == MCTP_BINDING_ASTLPC_MODE_HOST);

		layout->rx.offset = be32toh(hdr.layout.rx.offset);
		layout->rx.size = be32toh(hdr.layout.rx.size);
		layout->tx.offset = be32toh(hdr.layout.tx.offset);
		layout->tx.size = be32toh(hdr.layout.tx.size);
	}
}
				    ...
				    struct mctp_astlpc_layout *layout)
{
	...
	/* Flip the buffers as the names are defined in terms of the host */
	if (astlpc->mode == MCTP_BINDING_ASTLPC_MODE_BMC) {
		hdr.layout.rx.offset = htobe32(layout->tx.offset);
		hdr.layout.rx.size = htobe32(layout->tx.size);
		hdr.layout.tx.offset = htobe32(layout->rx.offset);
		hdr.layout.tx.size = htobe32(layout->rx.size);
	} else {
		assert(astlpc->mode == MCTP_BINDING_ASTLPC_MODE_HOST);

		hdr.layout.rx.offset = htobe32(layout->rx.offset);
		hdr.layout.rx.size = htobe32(layout->rx.size);
		hdr.layout.tx.offset = htobe32(layout->tx.offset);
		hdr.layout.tx.size = htobe32(layout->tx.size);
	}
...
static int mctp_astlpc_init_bmc(struct mctp_binding_astlpc *astlpc)
{
	 ...
	 * excluding the control space. Trim it to a multiple of 16 to meet the
	 * requirements of 12.17 Query Hop in DSP0236 v1.3.0.
	 */
	...
	rc = mctp_astlpc_layout_validate(&astlpc->layout);
	assert(rc == 0);
	...
	hdr = (struct mctp_lpcmap_hdr){
		...
		.layout = {
			.rx = {
				.offset = htobe32(astlpc->layout.tx.offset),
				.size = htobe32(astlpc->layout.tx.size),
			},
			.tx = {
				.offset = htobe32(astlpc->layout.rx.offset),
				.size = htobe32(astlpc->layout.rx.size),
			},
		}
	};
...
static int mctp_astlpc_negotiate_layout_host(struct mctp_binding_astlpc *astlpc)
{
	...
	astlpc_prinfo(astlpc, "Requesting Rx and Tx MTU of %" PRIu32 " bytes",
		      MCTP_BTU);
...
static uint32_t mctp_astlpc_calculate_mtu(struct mctp_binding_astlpc *astlpc,
					  struct mctp_astlpc_layout *layout)
{
	uint32_t active = MIN(astlpc->layout.rx.size, astlpc->layout.tx.size);
	uint32_t proposed = MIN(layout->rx.size, layout->tx.size);

	return MCTP_BODY_SIZE(ASTLPC_BODY_SIZE(MIN(active, proposed)));
}
...
static int mctp_astlpc_negotiate_layout_bmc(struct mctp_binding_astlpc *astlpc)
{
	...
	/* Use symmetric MTUs to pass constraints in rx/tx functions */
