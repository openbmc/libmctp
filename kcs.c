...
static int mctp_binding_kcs_tx(struct mctp_binding *b, struct mctp_pktbuf *pkt)
{
	...
	for (int i = 0; i < write_len; i++) {
		kcs_prdebug(kcs, "%s: data[%d]=0x%02x", __func__, i, send_buf[i]);
	}
	...
	for (int k = 0; k < write_len; k++) {
		...
		if (status & IPMI_KCS_COMMAND_DATA) {
			kcs_prerr(kcs,
				  "%s: returned data is not DATA", __func__);
			return -1;
		}
		...
		if (data != IPMI_KCS_CONTROL_CODE_READ) {
			kcs_prerr(
				kcs,
				"%s: received command is not IPMI_KCS_CONTROL_CODE_READ", __func__);
			return -1;
		}
	}
...
static int mctp_kcs_validate_data(struct mctp_kcs_header *hdr, int len)
{
	if (hdr->netFnAndLUN != MCTP_KCS_NETFN_LUN) {
		kcs_prerr(
			kcs,
			"%s: KCS binding header error! netFnAndLUN = 0x%02x, but should be 0x%02x", __func__,
			hdr->netFnAndLUN, MCTP_KCS_NETFN_LUN);
		return -1;
	}
	if (hdr->definingBody !=
		...
		kcs_prerr(
			kcs,
			"%s: KCS binding header error! definingBody = 0x%02x, but should be 0x%02x", __func__,
			hdr->definingBody,
			DEFINING_BODY_DMTF_PRE_OS_WORKING_GROUP);
		return -1;
	}
	if (hdr->len != (len - sizeof(struct mctp_kcs_header) - sizeof(struct mctp_kcs_trailer))) {
		kcs_prerr(
			kcs,
			"%s: KCS binding header error! len = 0x%02x, but should be 0x%02x", __func__,
			hdr->len,
	...
	struct mctp_kcs_trailer *tlr =
		(struct mctp_kcs_trailer *)((uint8_t *)(hdr + 1) +
					    hdr->len);
	if (PEC != tlr->PEC) {
		kcs_prerr(
			kcs,
			"%s: PEC error! Packet value=0x%02x, calculated value=0x%02x", __func__,
			tlr->PEC, PEC);
		return -1;
	}
	return 0;
...
int mctp_kcs_poll(struct mctp_binding_kcs *kcs)
{
	...
	if (data != IPMI_KCS_CONTROL_CODE_WRITE_START) {
		kcs_prerr(kcs, "%s: received command is not IPMI_KCS_CONTROL_CODE_WRITE_START", __func__);
		return -1;
	}
	...
	while (true) {
		...
		if (status & IPMI_KCS_COMMAND_DATA) {
			kcs_prdebug(kcs, "%s: received CMD=0x%02x", __func__, data);
			if (data != IPMI_KCS_CONTROL_CODE_WRITE_END) {
				kcs_prerr(kcs, "%s: received command is not IPMI_KCS_CONTROL_CODE_WRITE_END", __func__);
				return -1;
			}
			break;
		} else {
			kcs_prdebug(kcs, "%s: received DATA; buf[%d]=0x%02x", __func__, read_len, data);
			buf[read_len] = data;
			read_len++;
		}
	}
	...
	if (!pkt) {
		kcs_prerr(kcs,
			  "%s: unable to allocate pktbuf len 0x%x", __func__,
			  kcs_header->len);
	}
